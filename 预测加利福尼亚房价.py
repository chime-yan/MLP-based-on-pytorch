...
# 加利福尼亚房价数据集（测试回归）
regress_X = datasets.fetch_california_housing().data
regress_y = datasets.fetch_california_housing().target
x_train, x_test, y_train, y_test = model_selection.train_test_split(regress_X, regress_y, test_size=0.3)
...
model_path = 'california_housing_best_model.pth'
patience = 40
delta = 0.01
...
y_train = torch.tensor(y_train).to(torch.double)

x_test = torch.tensor(x_test).to(torch.double)
y_test = torch.tensor(y_test).to(torch.double)
...
# plt.ylabel("MSE")
# plt.xlabel("Epoch")
# plt.savefig("california_housing_losses.png")
# plt.close()
